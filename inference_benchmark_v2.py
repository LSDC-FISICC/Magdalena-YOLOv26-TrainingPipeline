...
# ─── Configuration ────────────────────────────────────────────────────────────
PT_MODEL     = "runs/classify/magdalena/plant_cls_ExG_HSV_Lab_Curation_100_lr_0.0005_4500_WD_0.04/weights/best.pt"
ONNX_MODEL   = "runs/classify/magdalena/plant_cls_ExG_HSV_Lab_Curation_100_lr_0.0005_4500_WD_0.04/weights/best.onnx"
ENGINE_MODEL = "runs/classify/magdalena/plant_cls_ExG_HSV_Lab_Curation_100_lr_0.0005_4500_WD_0.04/weights/best.engine"
TEST_DIR     = "sugarcane_exg_hsv_lab_v3-2-100.v1i.folder/test"
...
def run_pt(samples: list[dict], class_names: dict) -> list[dict]:
    print(f"\n[PT] Loading {PT_MODEL} ...")
    model = YOLO(PT_MODEL)

    dummy = Image.open(samples[0]["path"]).convert("RGB")
    dummy_tensor = INFER_TRANSFORM(dummy).unsqueeze(0)

    for _ in range(WARMUP_RUNS):
        model.predict(source=dummy_tensor, imgsz=IMGSZ, device=DEVICE_IDX, verbose=False)

    results = []
    for s in samples:
        img_pil = Image.open(s["path"]).convert("RGB")
        tensor  = INFER_TRANSFORM(img_pil).unsqueeze(0) #Resize images to fit with the trained model architecture (320x96)

        t0 = time.perf_counter()
        res = model.predict(source=tensor, imgsz=IMGSZ, device=DEVICE_IDX, verbose=False)[0] #img_pil normally
        t1 = time.perf_counter()

        pred_label = class_names[res.probs.top1]
        confidence = float(res.probs.top1conf)
        results.append({
            "true_label": s["true_label"],
            "pred_label": pred_label,
            "confidence": confidence,
            "correct":    pred_label == s["true_label"],
...
def run_onnx(samples: list[dict], class_names: dict) -> list[dict]:
    import onnxruntime as ort

    print(f"\n[ONNX] Loading {ONNX_MODEL} (CPUExecutionProvider) ...")
    ...
    def infer(img_pil: Image.Image) -> np.ndarray:
        #lb  = letterbox(img_pil, target=IMGSZ)
        lb = img_pil.resize((320, 96), Image.BILINEAR) #Resize images to fit with the trained model architecture (320x96)
        arr = np.array(lb, dtype=np.float32) / 255.0   # [H,W,C] in [0,1]
        arr = arr.transpose(2, 0, 1)[np.newaxis]        # [1,C,H,W]
        return session.run(None, {in_name: arr})[0][0]
...
def run_engine(samples: list[dict], class_names: dict) -> list[dict]:
    ...
    dummy = Image.open(samples[0]["path"]).convert("RGB")
    dummy_tensor = INFER_TRANSFORM(dummy).unsqueeze(0) #Resize images to fit with the trained model architecture (320x96)
    for _ in range(WARMUP_RUNS):
        model.predict(source=dummy_tensor, imgsz=IMGSZ, device=DEVICE_IDX, verbose=False)
    ...
    for s in samples:
        ...
        t0 = time.perf_counter()
        res = model.predict(source=tensor, imgsz=IMGSZ, device=DEVICE_IDX, verbose=False)[0] #img_pill normally
        t1 = time.perf_counter()
...
def plot_benchmark(backend_results: dict[str, list[dict]], samples: list[dict]) -> None:
    ...
    latencies   = {b: [r["latency_ms"] for r in v] for b, v in backend_results.items()}
    accuracies  = {b: np.mean([r["correct"]    for r in v]) * 100 for b, v in backend_results.items()}
    confidences = {b: [r["confidence"] for r in v] for b, v in backend_results.items()}
    throughputs = {b: 1000 / np.mean(latencies[b]) for b in backends}
    ...
    # ── Mean latency ──────────────────────────────────────────────────────────
    style_ax(ax_mean, "Mean Latency", ylabel="ms / image")
    ...
    # ── Latency box plot ──────────────────────────────────────────────────────
    style_ax(ax_box, "Latency Distribution", ylabel="ms / image")
    bp = ax_box.boxplot(
    ...
    style_ax(ax_line, "Per-Image Latency (all samples)",
             xlabel="Sample index", ylabel="ms")
    ...
    stat_lines = []
    for b in backends:
        n_ok = sum(r["correct"] for r in backend_results[b])
...
def main():
    ...
    pt_mean = np.mean([r["latency_ms"] for r in backend_results["PT"]])
    print("\n" + "=" * 65)
