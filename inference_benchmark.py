...
# ─── Configuration ────────────────────────────────────────────────────────────
PT_MODEL     = "runs/classify/magdalena/plant_cls_v3/weights/best.pt"
ONNX_MODEL   = "runs/classify/magdalena/plant_cls_v3/weights/best.onnx"
ENGINE_MODEL = "runs/classify/magdalena/plant_cls_v3/weights/best.engine"
...
def run_pt(samples: list[dict], class_names: dict) -> list[dict]:
    ...
    for _ in range(WARMUP_RUNS):
        model.predict(source=dummy, imgsz=IMGSZ, device=DEVICE_IDX, verbose=False)
    ...
    for s in samples:
        ...
        res = model.predict(source=img_pil, imgsz=IMGSZ, device=DEVICE_IDX, verbose=False)[0]
...
def plot_benchmark(backend_results: dict[str, list[dict]], samples: list[dict]) -> None:
    ...
    fig.suptitle(
        ...
        "Proyecto Magdalena  |  YOLO26s Classification  |  RTX 3070 Laptop",
